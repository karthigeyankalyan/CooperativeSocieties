...
class memberTransactions(object):
    ...
    def __init__(self, name, member_id, garment_type, district, society, wage_expected, advance_paid, intent_id,
        ...
        self.wage_expected = int(wage_expected)
        self.advance_paid = int(advance_paid)
        self.deductions = int(deductions)
        self.transaction_status = transaction_status
        self.remaining_amount = (int(wage_expected)-int(advance_paid)) if remaining_amount is None else remaining_amount
        self.thrift = int(thrift) if thrift is not None else thrift
        self.share = int(share) if thrift is not None else share
    ...
    def update_member_transaction(cls, name, member_id, garment_name, wage_expected, advance_paid, remaining_amount,
                                  ...
                                  thrift, share, issue_date, deductions):
        if deadline:
